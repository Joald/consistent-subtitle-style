...
- [ ] **Netflix font-size broken** — CSS `font-size: 150% !important` overrides Netflix's absolute pixel/vw size with a value relative to the parent element (~16px from body), producing tiny text. E.g. Netflix sets `font-size: 2.8vw` (≈36px on 1080p), we override with `150%` of parent = 24px. Fix: for platforms using CSS injection (no native API), compute the original font-size first and apply a multiplicative adjustment in px, OR use `transform: scale(1.5)` on the subtitle container which acts as a true multiplier regardless of the original size. This affects Netflix, Vimeo, and any CSS-only platform where the player sets absolute font sizes inline.
...
- [ ] **Per-setting site toggle (mini dropdown with platform icons)** — Replace the single global "Apply to" scope toggle with a mini dropdown positioned to the RIGHT of each setting dropdown. The mini dropdown shows platform icons (actual logos/favicons — YouTube play button, Netflix N, Dropout D, etc. — NOT text abbreviations like "YT"). Most-used platforms appear at top. Clicking a platform icon sets that setting's per-site override for that platform. Shows current platform's icon highlighted if override exists. This replaces the current "All Sites" / platform scope dropdown.
- [ ] **Use actual platform logos** — Replace all text abbreviation badges (YT, NF, D+, CR, etc.) with actual platform favicons/logos throughout the popup UI (per-site indicators in dropdown options, override badges, mini dropdown). Download 16×16 or 24×24 favicons from each platform and bundle in extension assets. Platforms: YouTube, Netflix, Dropout, Nebula, Prime Video, Max/HBO, Crunchyroll, Disney+, Vimeo.
...
- [ ] **Platform support indicator in popup** — When the popup opens on a supported site, show a banner/badge above the preview area indicating "✅ This site is supported" with the platform icon. On unsupported sites, show "⚠️ This site is not supported". Next to the indicator, show a small ℹ️ info icon that links to the platform-specific docs page.
- [ ] **Per-platform documentation pages** — Create an in-extension docs system (accessible from popup via ℹ️ icon or a "Help" link). For each of the 9 platforms, document: (1) how the extension works on that platform (CSS injection vs native API vs iframe bridge vs Shadow DOM), (2) which features are supported, (3) known limitations and workarounds (e.g. "Dropout: inline styles may reset on new subtitle lines", "Netflix: no native API, CSS-only", "YouTube: uses native captionWindow API for some settings"). Store docs as structured data in code (e.g. `platformDocs.ts`) so they're bundled with the extension. Render as a simple scrollable panel in the popup or a separate HTML page.
