...
async function run() {
  ...
  try {
    ...
    await setSiteOverride(browser, extId, 'netflix', siteSettings);
    await sleep(500);

    // Reload page + re-inject mocks to test per-site on init
    ...
    assert(
      ...
      'Per-site: netflix override fontColor=cyan applied',
      `got: ${siteCyan}`,
    );
    ...
    try {
      ...
      assert(popupInfo.selectCount === 10, 'Popup has 10 dropdowns (9 settings + preset selector)', String(popupInfo.selectCount));
